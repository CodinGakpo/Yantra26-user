...
function Profile() {
  ...
  // TEST-ONLY Aadhaar numbers (must match backend seed_aadhaar command)
  ...
  const getRandomUnusedAadhaar = () => {
    ...
    if (unused.length === 0) {
      // Recycle test pool for repeated local testing sessions.
      localStorage.removeItem("used_mock_aadhaars");
      return MOCK_AADHAAR_POOL[Math.floor(Math.random() * MOCK_AADHAAR_POOL.length)];
    }
